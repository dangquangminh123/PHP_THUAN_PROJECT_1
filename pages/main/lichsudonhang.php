  ...
  <tr>
    ...
    <td>
      <a href="main/indonhang.php?code=<?php echo $row['code_cart'] ?>">In Đơn hàng</a> 
   	...
   	<td>
   		<?php 
   		if($row['cart_payment']=='vnpay' || $row['cart_payment']=='momo' || $row['cart_payment']=='ONEPAY'){
   		?>
...
if(isset($_GET['congthanhtoan'])){
    ...
    <?php
		...
		$sql_onepay = mysqli_query($mysqli,"SELECT * FROM tbl_onepay WHERE code_cart='$code_cart' LIMIT 1");
		$row_onepay = mysqli_fetch_array($sql_onepay);
	?>
	<table class="table table-striped">
    <thead>
      <tr>
          <th>vpc_AccessCode</th>
          <th>ONEPAY_ID</th>
          <th>vpc_Amount</th>
          <th>vpc_OrderInfo</th>
          <th>Hình thức thanh toán</th>
          <th>Ngôn ngữ cổng thanh toán</th>
          <th>Phiên bản onepay</th>
          <th>Loại tiền tệ</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><?php echo $row_momo['vpc_AccessCode'] ?></td>
          <td><?php echo $row_momo['order_id'] ?></td>
          <td><?php echo $row_momo['vpc_Amount'] ?></td>
          <td><?php echo $row_momo['vpc_OrderInfo'] ?></td>
          <td><?php echo $row_momo['vpc_Command'] ?></td>
          <td><?php echo $row_momo['vpc_Locale'] ?></td>
          <td><?php echo $row_momo['vpc_Version'] ?></td>
          <td><?php echo $row_momo['vpc_Currency'] ?></td>
        </tr>
